...
2. Use the same public key to lock web servers as many as you need, and SSH into servers securely using the same private key.
    
3. How other users can get access to servers, they generate their own public and private keys, and the actual owner can copy the newly created public keys into server file, so others can access using their private keys.
    

### Let’s use Asymmetric Encryption in our scenario:

1. Generate keys at the server side.
...
11. CA verifies and signs the cert and sends it to you.
